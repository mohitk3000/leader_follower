...
while running:
    ...
    # LEADER UPDATE
    # ----------------------------
    leader = leader_step_with_bounds(leader, LEADER_V, LEADER_W)
    ...
    if time - last_sample_time >= SAMPLE_TIME:
        ...
        sampled_state = np.array([noisy_x, noisy_y, leader[2]])
    ...
    if target is not None:
        ...
        x_traj, vx = cubic_trajectory(follower[0], target[0], T, t_local)
        y_traj, vy = cubic_trajectory(follower[1], target[1], T, t_local)
        ...
        angle_error = wrap_angle(desired_theta - theta)
        w = 3.0 * angle_error

        follower = unicycle_step(follower, v, w)
    ...
    for _, s in sampled_targets:
        pygame.draw.circle(screen, (0, 120, 255), (int(s[0]), int(s[1])), 4)

    # robots
    draw_robot(leader[0], leader[1], leader[2], (0, 255, 0))
    draw_robot(follower[0], follower[1], follower[2], (255, 120, 0))
